...
__global__ void sync_guard_values(int blocks, float *alpha_guard_read, float *alpha_guard_write, float *beta_guard_read, float *beta_guard_write);
...
cudaMemset(dev_Lext12,0,DATASIZE*sizeof(float));
cudaMemset(dev_Lext21,0,DATASIZE*sizeof(float));
...
cudaMemset(dev_alpha_guard_read_1, EQUAL_GUARD_VALUE, blocks*8*sizeof(float));
cudaMemset(dev_alpha_guard_read_2, EQUAL_GUARD_VALUE, blocks*8*sizeof(float));
cudaMemset(dev_beta_guard_read_1, EQUAL_GUARD_VALUE, blocks*8*sizeof(float));
cudaMemset(dev_beta_guard_read_2, EQUAL_GUARD_VALUE, blocks*8*sizeof(float));
...
{	
	...
	sync_guard_values<<<(blocks*8)/64, 64>>>(blocks, dev_alpha_guard_read_1, dev_alpha_guard_write_1, dev_beta_guard_read_1, dev_beta_guard_write_1);
	sync_guard_values<<<(blocks*8)/64, 64>>>(blocks, dev_alpha_guard_read_2, dev_alpha_guard_write_2, dev_beta_guard_read_2, dev_beta_guard_write_2);
...
if(bid != 0)
{
	...
	//alpha_guard_copy[tid] = EQUAL_GUARD_VALUE;
}
if(bid != blocks-1)
{
	beta_guard_copy[tid] = beta_guard_read[(bid+1)*8+tid];
	//beta_guard_copy[tid] = EQUAL_GUARD_VALUE;
}
__syncthreads();
if(bid != 0)
{
...
if( bid != blocks-1)
{
	...
	for(k=block_size/8; k>0; k--)
	{
		for(j=0; j<=7; j++)
		{
			...
			lambda_0[j*8+tid] = gamma_000 + temp_beta[beta_state_0[tid]] + alpha[n*8+tid];
			lambda_1[j*8+tid] = gamma_111 + temp_beta[beta_state_1[tid]] + alpha[n*8+tid];
			//**********************************Lambda evaluation***************************
...
{
	//gamma_00 = (0.5f)*(LC*channel_x[block_size]+Lext[block_size])*(-1) + (0.5f)*LC*channel_y[block_size]*beta_encbit_0[tid];
	//gamma_11 = (0.5f)*(LC*channel_x[block_size]+Lext[block_size])*(1) + (0.5f)*LC*channel_y[block_size]*beta_encbit_1[tid];
	for(k=block_size/8; k>0; k--)
	{
		for(j=0; j<=7; j++)
		{
			n = k*8-j-1;
			if( bid == blocks-1 && n == block_size-1)
			{
			...
			lambda_0[j*8+tid] = gamma_00 + temp_beta[beta_state_0[tid]] + alpha[n*8+tid];
			lambda_1[j*8+tid] = gamma_11 + temp_beta[beta_state_1[tid]] + alpha[n*8+tid];
			//**********************************Lambda evaluation***************************
...
if(index < blocks*8)
{
	alpha_guard_read[index] = alpha_guard_write[index];
	beta_guard_read[index] = beta_guard_write[index];
}
}
