...
fp = fopen("gnucommands_1.txt","w");
fprintf(fp,"set term postscript eps enhanced color\n");
fprintf(fp,"set output \"snrvsber_plot_iter.ps\"\n");
...
fclose(fp);
system("gnuplot gnucommands_1.txt"); //using the script file gnucommands to plot
...
FILE *fp;
printf("Calling gnuplot\n");
fp = fopen("gnucommands_3.txt","w");
fprintf(fp,"set term postscript eps enhanced color\n");
fprintf(fp,"set output \"snrvsber_plot_block.ps\"\n");
fprintf(fp,"set xlabel \"SNR(in dB)\"\n");
fprintf(fp,"set ylabel \"BER(logscale)\"\n");
...
fprintf(fp,"\"ber_vs_snr_blocks_gnu.dat\" index %d:%d using 1:3 title \"# of blocks 1\" with lines\n", size, size);
fclose(fp);
system("gnuplot gnucommands_3.txt"); //using the script file gnucommands to plot
printf("Done plotting, go to the folder to see the plot\n");
}
...
FILE *fp;
printf("Calling gnuplot\n");
fp = fopen("gnucommands_2.txt","w");
fprintf(fp,"set term postscript eps enhanced color\n");
fprintf(fp,"set output \"snrvsber_plot_guard.ps\"\n");
fprintf(fp,"set xlabel \"SNR(in dB)\"\n");
fprintf(fp,"set ylabel \"BER(logscale)\"\n");
...
fclose(fp);
system("gnuplot gnucommands_2.txt"); //using the script file gnucommands to plot
printf("Done plotting, go to the folder to see the plot\n");
}
